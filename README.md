...
- Optional **movement trail**: a breadcrumb of the live robot's recent positions, drawn as a line on the map so you can see where it's actually been, not just where it is now. Client-side only — points are kept while **Live robot** is on, spaced at least 0.15 m apart, and bounded to the last 10 minutes / 3,000 points; turning either toggle off clears it.
